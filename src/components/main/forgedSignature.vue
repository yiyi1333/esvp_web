<template>
  <!--    上传区-->
    <el-card class="box-card mg-1 p-6">
        <el-text class="text-xl">上传伪造签名图像</el-text>
        <el-upload
                class="my-4"
                drag
...
export default {
    ...
    methods: {
        uploadSuccess(response) {
            ...
            // 将上传成功的图片写入到数据库中
            api.uploadForgedSignature(this.$store.state.userInfo.id, response[0]).then(res => {
                if (res.data.code == 200) {
                    ...
                    // 更新图片列表
                    api.getForgedSignatureList(this.$store.state.userInfo.id).then(res => {
        ...
        handleDelete(index, row) {
            ...
            api.deleteForgedSignature(row.id).then(res => {
                if (res.data.code == 200) {
                    this.$message({
                        message: res.data.msg,
                        type: 'success'
    ...
    created() {
        ...
        console.log('userInfo: ', this.$store.state.userInfo.id)
        //获取image列表
        api.getForgedSignatureList(this.$store.state.userInfo.id).then(res => {
            console.log(res.data)
            this.images = res.data.imageList
